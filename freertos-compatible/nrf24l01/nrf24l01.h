...
typedef struct
{
	...
	GPIO_ConfigTypeDef CSN;				/* Chip select pin */
	GPIO_ConfigTypeDef CE;				/* Chip enable pin */
	IRQ_GPIO_ConfigTypeDef INT;			/* Interrupt pin */
	...
	uint8_t InTxMode;			/* 1 if nRF24l01 Device is in TX mode, 0 otherwise */
	uint8_t Initialized;		/* 1 if initialized, unknown otherwise */
